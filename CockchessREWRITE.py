...
class CockchessBrain:
    ...
    def negamax(self, board, depth, alpha, beta, start, limit):
        ...
        if key in self.tt and self.tt[key]['d'] >= depth:
            entry = self.tt[key]
            if entry['f'] == 0: return entry['s']
            if entry['f'] == 1 and entry['s'] <= alpha: return entry['s']
            if entry['f'] == 2 and entry['s'] >= beta: return entry['s']

        if depth <= 0:
            return self.quiescence(board, alpha, beta, start, limit)
    ...
    def run(self):
        ...
        if self.mode == "INTERNAL":
            ...
            for d in range(1, 100):
                ...
                try:
                    ...
                    if curr:
                        best = curr
                        ev_str = f"{alpha/100:.2f}"
                        if abs(alpha) > 80000:
                            ev_str = f"MATE {(90000-abs(alpha)+d)//2}"
        ...
        # --- STOCKFISH MODE ---
            ...
            try:
                ...
                with eng.analysis(board, chess.engine.Limit(depth=target_depth)) as analysis:
                    for info in analysis:
                        ...
                        if "score" in info:
                            ...
                            if sc.is_mate():
                                ev = f"MATE {sc.mate()}"
                                raw = 10000 if sc.mate() > 0 else -10000
                            else:
                                ev = f"{sc.score()/100:.2f}"
                                raw = sc.score()
                            
                            pv_txt = " ".join([m.uci() for m in info.get("pv", [])[:4]])
                ...
                self.done.emit((None, f"SF Error: {str(e)}"))
        ...
        # --- ANALYSIS MODE ---
            ...
            if self.sf:
                try:
                    ...
                    pv = info["pv"][0].uci()
                    eng.quit()
                    self.done.emit((None, f"Best: {pv} | Eval: {ev}"))
                except Exception as e:
                    self.done.emit((None, f"Analyze Err: {str(e)}"))
            else:
